...
class ModelTutorPayment
{
    ...
    public function getAllPaymentDetails($tutorId): array
    {
        ...
        $this->db->query('SELECT d.id AS day_id, d.payment_status, d.is_completed, d.timestamp as date , c.id AS class_id , c.rate , m.name AS module, u.first_name, u.last_name, u.profile_picture 
    ...
    {
        $this->db->query("SELECT SUM(CASE WHEN payment_status = 0 THEN c.rate ELSE 0 END) AS Pending, SUM(CASE WHEN payment_status = 1  THEN c.rate ELSE 0 END) AS Earns FROM tutoring_class AS c JOIN day AS d ON d.class_id = c.id WHERE MONTH(d.timestamp) = MONTH(CURRENT_DATE()) AND YEAR(d.timestamp) = YEAR(CURRENT_DATE()) AND c.tutor_id = :tutorId ;");
