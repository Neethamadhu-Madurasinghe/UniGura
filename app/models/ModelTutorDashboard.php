...
class ModelTutorDashboard {
    ...
    public function countTutoringActiveClasses($id) {
        $this->db->query(' SELECT COUNT(CASE WHEN completion_status = 0 then 1 END) as active, COUNT(CASE WHEN completion_status = 1 THEN 1 END) as complete, COUNT(CASE WHEN completion_status = 2 THEN 1 END) as blocked FROM tutoring_class WHERE tutor_id =:id ');
