...
// If user clicks on Send tutor request button in any card, then show the popup to select time slots. 
bodyUI.addEventListener('click', async (e) => {
  
  if (e.target.classList.contains('send-request-btn')) {
    request.tutor_id = e.target.dataset.tutor;
...
function hideTimeTable() {
  ...
  popupTimeTableUI.classList.add('invisible');
}

// function to show timetable with last elements
function showTimeTable() {
  showLayoutBackground();
  popupTimeTableUI.classList.remove('invisible');
}
